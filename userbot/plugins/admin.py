...
async def pin(msg):
    ...
    if not to_pin:
        return await edit_delete(msg, "** رد على الرساله حتى اثبتها **", 5)
    ...
    await edit_delete(msg, "**تم التثبيت**", 3)
...
async def pin(msg):
    ...
    if not to_unpin and options != "الكل":
        await edit_delete(
            msg,
            " ** الرد على الرسالة لألغاء تثبيتها او استخدم**  `.الغاء تثبيت الكل` ",
            5,
        )
        return
...
async def kick(usr):
    ...
    if not user:
        ...
        return
    catevent = await edit_or_reply(usr, "**جاري الطرد**")
    try:
