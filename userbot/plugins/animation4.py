...
@bot.on(admin_cmd(pattern=f"انمي$", outgoing=True))
@bot.on(sudo_cmd(pattern=f"انمي$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
async def _(event):
    ...
    event = await edit_or_reply(event, "هلوو....")
    animation_chars = ["🐵", "🙉", "🙈", "🙊", "🖕‎🐵🖕", "**من الجيد رؤيتك صديقي....**"]
    for i in animation_ttl:
        await asyncio.sleep(animation_interval)
        await event.edit(animation_chars[i % 6])
...
@bot.on(admin_cmd(pattern=f"ايد$", outgoing=True))
@bot.on(sudo_cmd(pattern=f"ايد$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    animation_interval = 1
