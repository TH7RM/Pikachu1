...
DEFAULTUSERBIO = (
    str(DEFAULT_BIO) if DEFAULT_BIO else "- تعلقت بكِ حتى أصبحتِ وطناً لي ♡"
)
...
async def _(event):
    ...
    await event.client.send_message(
        event.chat_id, "**نسخوك/ج وانت النجر 😂😂**", reply_to=reply_message
    )
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#مستنسخ\nتم استنساخه بنجاح [{first_name}](tg://user?id={user_id })",
        )
...
async def _(event):
    ...
    await event.edit("** تم اعادة البروفايل الى وضعه الاصلي**")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, f"** تم اعادة البروفايل الى وضعه الاصلي**"
        )
