...
async def del_welcome(event):
    ...
    if rm_welcome_setting(event.chat_id) is True:
        ...
        await edit_or_reply(event, "**لايوجد ترحيب هنا ليتم حذفه💞**")


@bot.on(admin_cmd(pattern="لسته ترحيب$"))
@bot.on(sudo_cmd(pattern="لسته ترحيب$", allow_sudo=True))
async def show_welcome(event):
    if event.fwd_from:
        return
    cws = get_current_welcome_settings(event.chat_id)
    if not cws:
        await edit_or_reply(event, "**لا توجد رسالة ترحيب محفوظة هنا.**")
        return
    if cws.f_mesg_id:
        msg_o = await bot.get_messages(entity=BOTLOG_CHATID, ids=int(cws.f_mesg_id))
        await edit_or_reply(event, " ** ارحب حاليا بهذه الرساله للاعضاء الجدد**")
